...
SELECT * FROM models WHERE brand_id = (SELECT brand_id FROM brands WHERE name = 'Pontiac');
...
SELECT COUNT (brand_id) FROM models WHERE brand_id = (SELECT brand_id FROM brands WHERE name = 'Ford');
...
FROM brands AS b
WHERE b.brand_id NOT IN (SELECT brand_id FROM models);
